...
A *contact mode* assigns every interface (here: the two palms) either a chair patch or
"free", and records whether the chair itself rests on the floor. A *plan* is a sequence of
K modes plus the K+1 keyframe configurations between them, each keyframe carrying the
chair pose the plan wants there (FARO's kinematic sequence optimization solves one
configuration per transition, §II-E).
...
class ContactMode:
    """One contact mode: patch per hand (``None`` = free) + chair floor state."""
    ...
    def key(self) -> Tuple:
        ...
        return (self.left, self.right, self.grounded)
    ...
    def union(self, other: "ContactMode") -> "ContactMode":
        ...
        happen exactly at that configuration).
        """
        for side in HANDS:
...
def candidate_plans(
    ...
    if allow_push and dxy < 0.25 and dz < 0.02 and dyaw < 100.0:
        plans.append(push_plan(chair, start, goal))
    ...
    if dz < 0.02:
        ...
        for pair in ranked[:2]:
            plans.append(slide_plan(chair, start, goal, pair))
    for pair in ranked:
        plans.append(
            lift_plan(chair, start, goal, pair, lift_height=lift_height, n_transport=n_transport)
        )
    plans.append(rail_lift_plan(chair, start, goal, lift_height=lift_height,
                                n_transport=n_transport))
    # Past roughly half a turn (or a long carry) a single grasp cannot cover the whole
    # displacement with the feet planted, so offer regrasping variants as well. They are
    # tried last because each extra grasp is another set of filters to clear.
    n_seg = _segments_needed(dyaw, dxy)
    if n_seg > 1:
        for pair in ranked[:2]:
            plans.append(
                lift_plan(chair, start, goal, pair, lift_height=lift_height,
                          n_transport=max(1, n_transport - 1), segments=n_seg)
            )
    return plans
...
def plan_by_name(
    ...
    if name == "push":
        return [push_plan(chair, start, goal)]
    if name == "slide":
        ranked = sorted(PINCH_PAIRS, key=lambda p: -_pair_score(chair, p, start))
        return [slide_plan(chair, start, goal, pair) for pair in ranked]
    if name == "side_lift":
        ranked = sorted(PINCH_PAIRS, key=lambda p: -_pair_score(chair, p, start))
        n_seg = _segments_needed(
            np.degrees(goal.yaw() - start.yaw()), float(np.linalg.norm((goal.p - start.p)[:2]))
        )
        return [
            lift_plan(chair, start, goal, pair, lift_height=lift_height,
                      n_transport=n_transport, segments=seg)
            for seg in ({1, n_seg} if n_seg > 1 else {1})
            for pair in ranked
        ]
    if name == "rail_lift":
        return [rail_lift_plan(chair, start, goal, lift_height=lift_height,
                               n_transport=n_transport)]
