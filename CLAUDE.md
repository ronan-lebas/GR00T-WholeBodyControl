...
Originally, only the dex3 hands were supported. The brainco hands were added in a PR, as well as teleoperation using the Meta Quest, and object pose estimation using FoundationPose.  
We are focusing exclusively on the brainco hands version with the Meta quest teleoperation.  
...
Since hardware experiments are delayed, the focus for this branch of the repo is to use the existing stack to setup a test manipulation environment.